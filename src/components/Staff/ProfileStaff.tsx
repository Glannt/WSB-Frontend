...
const ProfileStaff: React.FC = () => {
  ...
  useEffect(() => {
    if (staff) {
      ...
      if (staff) {
        const formattedDateOfBirth = formatDate(staff.dateOfBirth);
        staff.dateOfBirth = formattedDateOfBirth;
  ...
  const onSubmit = async (formData: Staff) => {
    if (isModified) {
                ...
                <DatePicker
                  ...
                  onChange={(dateValue) => {
                    const formattedDate = dateValue
                      ? `${dateValue.year}-${String(dateValue.month).padStart(2, '0')}-${String(dateValue.day).padStart(2, '0')}` // Format as YYYY-MM-DD
                      : '';
